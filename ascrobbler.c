...
struct artist *lookup_artist(struct artist **index, int n, int artist_id)
{
	...
	{
	middle = (left + right) / 2;
		if( index[middle]->id == artist_id )
			result = index[middle];
		else if( index[middle]->id < artist_id )
			left = middle+1;
		else
			right = middle-1;
	}
...
{
	struct list *current = artists;
	struct artist *compare = (struct artist *)current->val;	 		//enables the program to compare artist to input artist 
	int i;
	
	for (i=0; i<1000000; i++)										//for loop used to prevent segmentation fault - temporary
	//while ( a != NULL && artists != NULL )
		{
		compare->similarity = similarity(a, compare);
		//printf("%.10f\n", compare->similarity);					//to check/examine values for similarity created by function
		current = current->next;
		compare = (struct artist *)current->val;
		}
	
	return sort(current, a_leq_jc);		
}	*/

struct list *sort_similarity(struct artist *a, struct list *artists) 
{	
	struct list *current = artists;
	struct artist *compare = NULL;									//enables the program to compare artist to input artist 
	print_artist(a);
